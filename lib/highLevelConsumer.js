            ...
            operation.attempt(function (currentAttempt) {
                self.rebalanceAttempt(oldTopicPayloads, function (err) {
                    ...
                    if (err) {
                        ...
                        self.updateConsumer(function (err) {
                            if (err) {
...
HighLevelConsumer.prototype.updateConsumer = function (cb) {
    this.client.zk.updateConsumer(this.options.groupId, this.id, this.payloads, function (err) {
        if (err) return cb(err);
        cb();
    });
    this.client.zk.listConsumers(this.options.groupId);
